...
module.exports = async (client, msg, user) => {
    ...
    const questions = [
        {
            ...
            one: "**Quel est ton nom ?**",
            two: "**Quel est le nom de ton studio ?**"
        },
        {
            step: 'description',
    ...
    for (let i = 0, cancel = false; i < questions.length && cancel === false; i++) {
        ...
        let questionMsgOne = questions[i].one;
        let questionMsgTwo = questions[i].two;
        ...
        try {
            ...
            if (hasOptions) {
                const msg = await dmChannel.awaitMessages(m => m.channel.type === 'dm', { max: 1, time: 5 * 60 * 1000, errors: ['time'] });
                const msgContentOption = msg.last().content.trim();
                ...
                if (msgContentOption == 1) {
                    console.log("Choisi l'option 1")
                    await dmChannel.send(questionMsgOne);
                }
                else if (msgContentOption == 2) {
                    console.log("Choisi l'option 2")
                    await dmChannel.send(questionMsgTwo);
                }
            }
